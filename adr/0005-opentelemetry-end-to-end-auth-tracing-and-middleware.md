...
- **Scope**: `@chief-strategist-j/shared-infra/tracing` centralized engine, `@observability/auth` tracing infrastructure, HTTP middleware, AsyncLocalStorage context propagation, W3C trace propagation, CORS wildcard handling, and Grafana Tempo ingestion
...
1. **Centralized Engine in `@chief-strategist-j/shared-infra/tracing`**:
   - Extracted all OpenTelemetry provider initialization (`initNodeTracing`), tracer factory (`getTracer`), span execution wrappers (`withSpan`), HTTP server middleware (`runWithHttpTracing`), messaging tracer (`CentralMessagingTracer`), and base handler abstractions (`BaseTracedKafkaHandler`) into `@chief-strategist-j/shared-infra/tracing`.
...
└── 1. RawAuthApiClient.execute('signIn', payload) [packages/node/web-app/src/lib/auth-client.ts]
    ...
    └── Injects Header: x-correlation-id: corr-full-consumer-100
        ...
        └── 2. server.ts :: http.createServer handler [packages/node/auth/src/server.ts]
            └── 3. runWithHttpTracing(req, res) [@chief-strategist-j/shared-infra/tracing/http-middleware.ts]
                ...
                └── Start Active SERVER Span: `HTTP POST /api/v1/auth/sign-in`
                    ...
                    └── 4. router.ts :: AuthRestV1Router.route("POST", "/api/v1/auth/sign-in") [packages/node/auth/src/api/rest/v1/router.ts]
                        └── withSpan("REST POST /api/v1/auth/sign-in") [@chief-strategist-j/shared-infra/tracing/tracer.ts]
                            ...
                            └── 5. user-auth.service.ts :: UserAuthDomainService.signIn(input) [packages/node/auth/src/features/auth/services/user-auth.service.ts]
                                ...
                                │   └── withSpan("DB SELECT findUserByEmail", kind: CLIENT) [@chief-strategist-j/shared-infra/tracing/tracer.ts]
                                ...
                                │   └── withSpan("Argon2id Password Check") [@chief-strategist-j/shared-infra/tracing/tracer.ts]
                                │
                                ...
                                │   └── withSpan("DB INSERT recordAuditLog", kind: CLIENT) [@chief-strategist-j/shared-infra/tracing/tracer.ts]
                                ...
                                └── 9. auth-event.producer.ts :: publishUserSignedIn(payload) [packages/node/auth/src/shared/messaging/producers/auth-event.producer.ts]
                                    └── CentralMessagingTracer.createProducerSpan("auth.events.v1", "USER_SIGNED_IN") [@chief-strategist-j/shared-infra/tracing/messaging-tracer.ts]
                                        ...
                                        └── Publish to Kafka Topic `auth.events.v1` (Port 31414)
                                            ...
                                            └── 10. auth-event.consumer.ts :: subscribeToTopic('auth.events.v1') [packages/node/auth/src/shared/messaging/consumers/auth-event.consumer.ts]
                                                ├── CentralMessagingTracer.createConsumerSpan(event) [@chief-strategist-j/shared-infra/tracing/messaging-tracer.ts]
                                                │   └── Start CONSUMER Span: `Kafka CONSUMER USER_SIGNED_IN`
                                                │
                                                └── 11. UserSignedInHandler extends BaseTracedKafkaHandler [@chief-strategist-j/shared-infra/tracing/traced-handler.ts]
    ...
    └── 12. SimpleSpanProcessor -> OTLPTraceExporter [@chief-strategist-j/shared-infra/tracing/tracer.ts]
